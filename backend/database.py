...
import sqlite3, os, math, threading
...
# H-31: serialise schema migrations across threads. ThreadingHTTPServer
# dispatches every request on a fresh thread, so concurrent /api/init or
# /api/seed calls could race on ALTER TABLE and one would raise
# OperationalError. We expose a single lock so init_db is reentrant-safe.
_INIT_LOCK = threading.Lock()
def _has_unique_constraint(conn, table, columns):
    """Return True if `table` already has a UNIQUE constraint covering `columns`.

    SQLite doesn't expose UNIQUE constraints by name in a portable way, so
    we look at the CREATE TABLE statement for the column signature.
    """
    row = conn.execute(
    ...
    if not row or not row[0]:
        return False
    sql = row[0].upper()
    sig = "(" + ", ".join(c.upper() for c in columns) + ")"
    return sig in sql


def _add_unique_constraint(conn, table, new_columns, all_columns):
    """Rebuild `table` with extra UNIQUE(`new_columns`) preserving data.

    SQLite has no ALTER TABLE ADD CONSTRAINT, so we:
      1. RENAME old to a scratch table
      2. CREATE the new schema with the desired UNIQUE
      3. INSERT OR IGNORE rows from scratch into the new table (collapses
         pre-existing duplicates to a single row)
      4. DROP scratch
    All inside a single transaction so a crash leaves the old table intact.
    """
    scratch = "_" + table + "_old"
    ...
    for col, decl in col_decls:
        if col in all_columns:
            new_sql_lines.append("            " + col + " " + decl + ",")
    new_sql_lines.append("            UNIQUE(" + ", ".join(new_columns) + ")")
    new_sql_lines.append("        )")
    new_sql = chr(10).join(new_sql_lines)
    conn.executescript(
        "BEGIN;" + chr(10)
        + "ALTER TABLE " + table + " RENAME TO " + scratch + ";" + chr(10)
        + new_sql + ";" + chr(10)
        + "INSERT OR IGNORE INTO " + table + " (" + cols_csv + ") "
            "SELECT " + cols_csv + " FROM " + scratch + ";" + chr(10)
        + "DROP TABLE " + scratch + ";" + chr(10)
        + "COMMIT;"
    )


def init_db():
    # H-31: serialise so concurrent /api/init + /api/seed can't race on ALTER
    # TABLE / CREATE INDEX.
    with _INIT_LOCK:
        conn = get_conn()
        try:
                ...
                if "phone" not in cols:
                    try:
                        ...
                        if "duplicate column" not in str(e):
                            raise
            # H-19: rebuild tables with UNIQUE constraints so INSERT OR IGNORE
            # actually skips duplicates (otherwise AUTOINCREMENT id never
            # collides and seed.py duplicates rows on every run).
            if not _has_unique_constraint(conn, "attractions", ("name", "city")):
                _add_unique_constraint(
                    conn,
                    "attractions",
                    new_columns=("name", "city"),
                    all_columns=("id", "name", "rating", "city", "province",
                                 "address", "lat", "lng", "description",
                                 "recommend", "source", "phone", "created_at"),
                )
            if not _has_unique_constraint(conn, "foods", ("name", "city", "shop_name")):
                _add_unique_constraint(
                    conn,
                    "foods",
                    new_columns=("name", "city", "shop_name"),
                    all_columns=("id", "name", "city", "province", "lat", "lng",
                                 "description", "address", "shop_name",
                                 "recommend_dish", "phone", "source", "created_at"),
                )
            conn.commit()
        finally:
            conn.close()
...
def _dedup_foods(items):
    """Merge food records that are literally the same shop in the same city.

    Key: (city, shop_name or name) using strict equality (no name normalisation).

    Previously this function ran shop_name through _normalize_name(), which
    strips parenthetical content such as "(平安里西大街店)" — that collapsed
    distinct branches of the same chain (e.g. "潮乡原品汕头牛肉丸(细岗店)"
    and "潮乡原品汕头牛肉丸(西丽店)") into a single record and hid every
    branch from the map. Switched to strict key so each branch stays visible.
    """
    if not items:
        return items
    by_key = {}
    for it in items:
        ...
        key = ((it.get("city") or "").strip(), shop or name)
